              ...
              child: FutureBuilder<List<Presentation>>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    child = const Center(
                      key: ValueKey("abstracts-loading"),
                      child: CircularProgressIndicator(),
                    );
                  } else if (snapshot.hasError) {
                    child = Center(
                      key: const ValueKey("abstracts-error"),
                      child: Padding(
                    ...
                    final presentations = _filterPresentations(
                      snapshot.data ?? [],
                    );

                    if (_searchText.isEmpty) {
                      child = const Center(
                        key: ValueKey("abstracts-empty-search"),
                        child: Text("Search for a topic or author"),
                      );
                    } else if (presentations.isEmpty) {
                      child = const Center(
                        key: ValueKey("abstracts-no-results"),
                        child: Text("No abstracts found."),
                      );
                    } else {
                      child = ListView.builder(
                        key: const ValueKey("abstracts-results"),
